...
int main(){
	...
	TCNT0=111; //타이머0을 가지고 0.0025초 오버플로 인터럽트를 만들기 위해서 14.7456MHZ인데 TCCRO의 설정이 256분주
	TIMSK=0x01; //타이머0 오버플로 인터럽트를 허가함
	init_DEVICES();	
	MotorStop();
	...
	{ 
		...
		OCR3B = 500;
		if(flag){  //1초 경과
            RPM_L=0.3*M_L/0.036;// rpm=(60*m)/(주기*분해능)=(60*m)/(1s*1000) // 나누기 기어비 
            RPM_R=0.3*M_R/0.036;
